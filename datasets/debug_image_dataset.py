...
class DebugImageDataset(DatasetBase):
    ...
    def save_debug_images(self, entry_dict):
        ...
        for key, value in generators.items():
            ...
            if self.debug_image_type == "single_image":
                if not os.path.exists(os.path.join(self.debug_image_folder, current_id[:-8]).replace('\\','/')):
                    os.makedirs(os.path.join(self.debug_image_folder, current_id[:-8]).replace('\\','/'))
                file_name = os.path.join(self.debug_image_folder, current_id + '_' + key + '.png').replace('\\','/')
            else:
                file_name = os.path.join(self.debug_image_folder, current_id + '_' + key + '.mha').replace('\\','/')
