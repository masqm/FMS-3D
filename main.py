...
def get_dataset_parameters(dataset_name):
    ...
    normalization_consideration_factors = {'DIC-C2DH-HeLa': (0.001, 0.001),
                                           ...
                                           'Fluo-N3DH-CHO': (0.001, 0.001),
                                           'Fluo-N3DH-SIM+': (0.001, 0.001)}
...
def image_sizes_for_dataset_name(dataset_name):
    train_image_size = {'DIC-C2DH-HeLa': [256, 256],
                        ...
                        'Fluo-N3DH-CHO': [256, 256],
                        'Fluo-N3DH-SIM+': [256, 256]}
    ...
    test_image_size = {'DIC-C2DH-HeLa': [256, 256],  # * 2
                        ...
                        'PhC-C2DL-PSC': [720, 576],
                        'Fluo-N3DH-CHO': [256, 256],
                        'Fluo-N3DH-SIM+': [256, 256]}  # * 2
...
class MainLoop(MainLoopBase):
    def __init__(self, cv, dataset_name, num_embeddings, actual_network, network_parameters, num_frames, learning_rate=0.00001):
        ...
        if self.cv == 'train_all':
            ...
            self.max_iter = 40000
        self.test_iter = self.max_iter // 1
        self.disp_iter = 100
        ...
        self.base_folder = os.path.join('E:/code/data/myfile/celltrackingchallenge/trainingdataset/', self.dataset_name).replace('\\','/')
        self.base_folder_test = os.path.join('E:/code/data/myfile/celltrackingchallenge/challengedataset/', self.dataset_name).replace('\\','/')
        self.output_base_folder = os.path.join('E:/code/data/myfile/output2/', self.dataset_name).replace('\\','/')
        #self.output_folder = os.path.join(self.output_base_folder, folder_string + '_' + self.output_folder_timestamp()).replace('\\','/')
        self.output_folder = os.path.join(self.output_base_folder, folder_string + '_' + self.output_folder_timestamp()).replace('\\','/')
    ...
    def test_folder(self, base_folder, dataset, name, update_loss):
        ...
        """
        setup_base_folder = os.path.join(base_folder, 'setup').replace('\\','/')
        video_frame_list_file_name = os.path.join(setup_base_folder, 'frames.csv').replace('\\','/')
        iterator = IdListIterator(os.path.join(setup_base_folder, 'video_only_all.csv').replace('\\','/'), random=False, keys=['video_id'])
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        loop = MainLoop(2, dataset, 16, UnetIntermediateGruWithStates2D, network_parameters, num_frames, 0.0001)
        loop.run()
