...
def get_seg_tra_image_list_2d(folder):
    ...
    while True:
        num_str = f'{index:03}'
        #file_name_seg = os.path.join(folder, 'SEG/man_seg' + num_str + '.tif').replace('\\','/')
        file_name_seg = os.path.join(folder, 'SEG/man_seg' + num_str + '.tif').replace('\\','/')
        try:
        ...
        file_name_tra = os.path.join(folder, 'TRA/man_track' + num_str + '.tif').replace('\\','/')
        try:
...
def get_input_image_list_2d(folder):
    ...
    while True:
        num_str = f'{index:03}'
        file_name = os.path.join(folder, 't' + num_str + '.tif').replace('\\','/')
        try:
...
def process_seg_folder(folder, output_folder_prefix):
    ...
    print(split)
    output_folder = os.path.join(output_folder_prefix, split[-3], split[-2], split[-1], 'MERGED').replace('\\','/')
    print(output_folder)
    ...
    for i, (seg_im, tra_im) in enumerate(zip(seg, tra)):
        ...
        if seg_im is not None:
            ...
            utils.io.image.write(merged_im, os.path.join(output_folder, file_name).replace('\\','/'))
        else:
            tra_im_np = utils.sitk_np.sitk_to_np(tra_im)
            stacked_np.append(tra_im_np)
            utils.io.image.write(tra_im, os.path.join(output_folder, file_name).replace('\\','/'))
    ...
    print('save')
    utils.io.image.write(stacked, os.path.join(output_folder, 'stacked.mha').replace('\\','/'))
...
def process_input_folder(folder, output_folder_prefix):
    ...
    split = folder.split(os.sep)
    print(split)
    output_folder = os.path.join(output_folder_prefix, split[-3], split[-2], split[-1]).replace('\\','/')
    print(output_folder)
    ...
    for i, im in enumerate(im_list):
        ...
        utils.io.image.write(im, os.path.join(output_folder, file_name).replace('\\','/'))
...
def main():
    dataset_base_folder = 'E:/code/data/my_folder/'

    #seg_folders = glob.glob(dataset_base_folder + 'trainingdataset/*2d*/0?_gt/')
    seg_folders = glob.glob(dataset_base_folder + 'trainingdataset/Fluo-N3DH-CHO/0?_gt/')
    ...
    image_folders = sorted(glob.glob(dataset_base_folder + '*/Fluo-N3DH-CHO/0?/'))
    ...
    #my_folder = 'e:/code/data/mia/'
    #print(my_folder)
    #process_seg_folder(my_folder, 'celltrackingchallenge')
