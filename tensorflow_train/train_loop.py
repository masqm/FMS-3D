...
class MainLoopBase(object):
    ...
    def load_model(self):
        if self.load_model_filename is not None:
            ...
            model_filename = os.path.join(self.output_folder, 'weights/model-' + str(self.current_iter)).replace('\\','/')
    ...
    def output_folder_for_current_iteration(self):
        return os.path.join(self.output_folder, 'iter_' + str(self.current_iter)).replace('\\','/')

    def output_file_for_current_iteration(self, *args):
        return os.path.join(self.output_folder, 'iter_' + str(self.current_iter), *args).replace('\\','/')
    ...
    def snapshot(self):
        ...
        save_path = self.saver.save(self.sess, os.path.join(self.output_folder, 'weights/model').replace('\\','/'), global_step=self.current_iter)
    ...
    def initLossAggregators(self):
        ...
        self.train_loss_aggregator = SummaryHandler(self.sess,
                                                    ...
                                                    'train',
                                                    os.path.join(self.output_folder, 'train').replace('\\','/'),
                                                    os.path.join(self.output_folder, 'train.csv', ).replace('\\','/'))
        ...
        self.val_loss_aggregator = SummaryHandler(self.sess,
                                                  ...
                                                  'test',
                                                  os.path.join(self.output_folder, 'test').replace('\\','/'),
                                                  os.path.join(self.output_folder, 'test.csv').replace('\\','/'))
