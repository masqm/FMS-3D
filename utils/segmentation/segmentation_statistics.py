...
class SegmentationStatistics(object):
    ...
    def save_metric_values(self, metric_key):
        ...
        utils.io.text.save_dict_csv(metric_dict, os.path.join(self.output_folder, metric_key + '.csv').replace('\\','/'), header)

    def save_metric_summaries(self, metric_summaries):
        if self.output_folder is None:
            return
        file_name = os.path.join(self.output_folder, 'summary.csv').replace('\\','/')
